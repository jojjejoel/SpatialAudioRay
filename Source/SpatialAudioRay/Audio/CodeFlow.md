...
Cached edges are snapshotted into `PendingValidCachedPoints` at cycle 0. They pay off through `ApplyCacheFullnessRayScale`, which interpolates the ray budget toward `FullCacheRayScale` by how full the cache is, floored at one ray. The reasoning is that a saturated cache can only improve by displacing an entry, which is rank-gated and hysteresis-damped, so most of a full-budget sweep into a full cache is speculative. It scales rather than stops because a cache that stopped searching could never find the corner that would displace something. `CountHeldEdges` excludes relayed and evicting entries: a relay is an audible stopgap rather than a found path, so it must not throttle the search that would replace it.

Per-direction skipping was tried twice and removed both times, first as a cone test and then as an index, most recently 2026-08-02. Measurement is why: with edges clustered around one opening they share a nearest Fibonacci direction, so the saving collapsed to about one ray regardless of cache size, which is the same emergent behaviour the cone test was removed for. It also suppressed the finds that improve a cached edge, since a ray landing inside the merge radius bypasses rank scoring and lets the shorter travelled path win outright, and skipping put a permanent blind spot exactly there.
...
**Listener movement never evicts anything**, and neither does source movement directly. Listener-side validity is entirely Phase 0's job; the source side belongs to the shortest-path recheck below, which re-anchors rather than evicting.
...
Unverified segments are skipped, meaning raw crawl or bounce hops the string pull could not shortcut past. Those were blocked at discovery by construction, so tracing them would evict every ordinary multi-corner path on sight rather than catching genuine geometry changes.

The first segment is special: it is retraced from the **live** source position rather than the stored one, and a clear result re-anchors the entry, moving `ShortestPath[0]` and re-measuring `PathDist` and `GeomDist`. That is what makes source movement a measured quantity rather than a distance heuristic, and it is why no move-threshold eviction exists. Re-measuring is the load-bearing half: `MergeIntoSameCorner` only accepts a find that is shorter, so an entry left reporting the distance from a position the source has left could never be corrected by a later sweep, and `PathDist` feeds path attenuation directly. When the first segment is unverified it cannot be retraced at all, so an entry whose source has moved off its anchor is evicted instead.

This recheck is the only guard on the source side: Phase 0 watches the listener leg and rank hysteresis discards the worse-ranking re-finds a closed path produces. Any blocked segment triggers a source-side eviction fade and sweep request, flagged so Phase 0's clear-restore cannot resurrect it, since the listener leg is usually still clear when the source leg closes and the restore would outpace the fade every interval.
...
Relay conversion is what actually piles them up, because it is inherently simultaneous: several edges bending around the same physical corner lose LoS on the same tick, each converts along its own edge-to-relay leg, and those legs all terminate at that one corner. Duplicates are not free. Each costs a cache slot, counts toward the cache fullness that throttles the sweep budget through `FullCacheRayScale`, and, since `Math::ClusterEdgePoints` sums member weights into `Cluster.TotalWeight`, takes a larger share of the voice mix than a genuinely distinct opening gets.
...
The relay rescue runs on total listener LoS loss, before evicting. The edge tries to survive routed through `LastLoSListenerPos`, the most recent listener position whose Phase 0 saw it, or the clear fan point when only the fan saw it. `SubmitRelayRescueTraces` submits both legs, edge to relay and relay to current listener, forward and reverse each, on `RescueHandles`/`bRescuePending`; `TickRelayRescueReadback` rules the following tick and the edge stays alive with `bRelayed` set if all four came back clear. `RelayPoint` and `RelayDist` are frozen at rescue time so the extended path stays listener-independent. There is one relay level: a relayed edge whose relay goes dark evicts normally.

The edge is deliberately left playing at full weight for the tick the traces are in flight, rather than starting the fade pre-emptively and cancelling it on success. A doomed edge only loses one frame off a fade it was about to start anyway, a rescued one never dims at all, and pre-fading would break outright at `CachedEdgeEvictionFadeTime` 0, where `TickEvictionFade` removes the entry on the next tick before any readback could land. Two interactions the synchronous version could not have: Phase 0 does not resubmit while a rescue is pending, and a rescue that lands on an edge the source-movement check evicted meanwhile is dropped, since the listener legs cannot speak for a stale source side.
...
**The source components.** Every component tagged `AudioComponentSource`, since an object can carry several co-located sounds all writing the shared diffraction bus, receives `CurvedOcclusion` through `OcclusionParamName`. Each MetaSound graph shapes its own volume and filtering from it continuously, with no external crossfade. A destroyed component's stale entry is pruned in the same loop. `ReadAttenuationSettings` uses the widest-range source for `AttenuationInnerRadius` and for the ray range, which is that source's audible range times `MaxRayDistanceScale`.
...
The layer split mirrors `Audio/`'s own. `Voice/NPCVoiceLogic.h` holds every scheduling decision as pure functions over explicit state, in the `VoiceLogic` namespace, following the same convention as `Math.h`. `Voice/NPCVoiceTypes.h` holds the bank row and the resolved runtime line, and `Voice/NPCVoiceState.h` the scheduler state structs those functions mutate, `FNPCVoiceEffortHysteresis`, `FNPCVoicePlaybackState`, `FNPCVoiceTransitionState`, `FNPCVoiceSightState` and `FNPCVoiceAcousticState`. The component keeps only engine wiring, resolving sibling components, loading the DataTable, and calling `Play`, `FadeOut`, `SetWaveParameter` and `SetAttenuationOuterRadius`. Every decision is therefore unit-testable without a component, world or audio device, under `SpatialAudioRay.Voice.*`.

### Effort from effective acoustic distance

Per tick the layer maps effective acoustic distance to a vocal effort, `ENPCVoiceEffort` from whisper to shout, with bands selecting inversely so close is a whisper and far is a shout.
...
The blend takes a detour floor, and the voice passes `OcclusionShiftThreshold`, the same threshold that calls the listener hidden. Below it, effort follows the straight line exactly. Above it, the route phases in across the remaining range, reaching the full path at total occlusion. Edge caching finds routes well before the source is hidden, since the pre-sweep band pre-warms the cache during partial LoS, and blending those in from occlusion 0 charged the NPC for a detour the sound was not taking. It strained at a listener it could plainly see while simultaneously saying "I can see you" lines. One threshold for both keeps effort and content from disagreeing about whether the detour is real, and the remap means nothing jumps at the crossing. Occlusion still never shifts the effort directly.

The effort commits through dwell-time hysteresis, and lines come from a `FNPCVoiceLineRow` DataTable generated by `Tools/VoiceGen/`, with content keyed to the acoustic situation, cooldown groups and no immediate repeats.
...
There is deliberately no "visible but far" context. For a visible listener the effective distance is essentially the straight line, so the effort already partitions the visible half by distance and a `Clear` line at shout is by construction a distant one. The category that used to do this restated the effort and carried its own threshold that had to be hand-synced with a band edge.
...
A playing line is never modified mid-flight, and effort changes apply to the next line, with one exception. `TickSightReaction` handles three moments worth reacting to, ranked by `EvaluateBargeIn`: direct line of sight breaking, sight returning, and the committed effort drifting at least `TransitionEffortDelta` steps from the effort the playing line started at. The line is cut with a short declick fade, and one tick later a line from the reason's category fires through `FindBargeInLine`, which prefers the exact target effort then the nearest rendered one, followed after `PostTransitionLineDelay` by a full line at the new effort.
...
Effort sets audible reach, derived from the same distance bands that select it rather than authored separately. An effort exists to be heard across its own band, so a whisper only has to carry as far as the distance at which the NPC would escalate to conversational anyway. `GetEffortReachDistance` returns the effort's band maximum times `EffortReachHeadroom`, and each `PlayLine` passes it to `USpatialAudioComponent::SetAttenuationOuterRadius`, which converts it to a `FalloffDistance` scale through `Math::ComputeFalloffScaleForOuterRadius`, applied to the source components and the live virtual pool.

Headroom above 1 keeps an effort audible slightly past its band edge, because the effort only commits after `EffortDwellTime` and a line in flight finishes at its starting effort, so the listener is regularly a little past the boundary while an older effort is still speaking. Two clamps bound the result: the attenuation's inner radius is a hard floor, since volume is full inside it, and the asset's own range is the ceiling, because the captured ray and LoS ranges stay at base scale and a sound audible past them would have no diffraction paths to play through. Shout has no band maximum and defers to the asset's authored range, being the anchor everything else scales down from.
