...
Per tick it maps the **effective acoustic distance** to a vocal-effort bucket (`ENPCVoiceEffort`, whisper → shout; bands select inversely — whisper close, shout far). Effective distance comes from `USpatialAudioComponent::GetEffectiveAcousticDistance`: the straight line while clear, the shortest cached diffraction route (min over non-evicting cached edges of `EffectivePathDist()` + `EffectivePoint()`→listener — the most favorable path, same min-not-average rule the occlusion accumulators follow) while occluded, blended by smoothed `CurrentOcclusion` (`Math::ComputeEffectiveAcousticDistance`) — so a listener just around a small corner gets at most a small step up while one three rooms deep walks the bands toward Shout, and effort steps stay graded even though the occlusion fraction itself swings near-binarily around a corner. Occlusion no longer shifts the bucket directly (`OcclusionShiftThreshold` only switches content category). The bucket commits through dwell-time hysteresis, and lines are scheduled from a `FNPCVoiceLineRow` DataTable (bank generated by `Tools/VoiceGen/`): category keyed to occlusion state (Clear vs Occluded content), cooldown groups, no-immediate-repeat. Playback injects the line's wave into the owner's voice `UAudioComponent` (tagged both `AudioComponentSource` — so the pipeline above feeds it bus + occlusion — and `NPCVoiceAudio`) via `SetWaveParameter` before `Play`, so a voice line diffracts and muffles exactly like any other co-located sound. A playing line is never modified mid-flight — bucket changes apply to the next line — with one exception: **transition barge-in** (`TickTransitionBargeIn`). When the committed bucket drifts ≥ `TransitionBucketDelta` steps from the bucket the playing line *started* at, the line is cut with a short declick fade (`FadeOut`), and one tick later a Transition-category line fires in the jump's direction (`ENPCVoiceTransitionDir` Closer/Farther on the row; `FindTransitionLine` prefers the exact target bucket, then the nearest rendered one) — "oh, you're right here" / "hey, you're running off!" — followed after `PostTransitionLineDelay` by a full line at the new effort. Guards: per-barge-in cooldown, minimum remaining line time, transitions never interrupt transitions, and a bank with no Transition rows keeps the old wait-for-line-end behavior. Effort also sets **audible reach**: each `PlayLine` calls `USpatialAudioComponent::SetAttenuationFalloffScale` with the bucket's falloff scale (whisper ~0.15 → shout 1.0), rescaling the attenuation `FalloffDistance` of the source components and the live virtual pool relative to base — the wavs stay at one LUFS (effort = timbre), so reach differences live entirely in the engine attenuation. The base attenuation is authored for shout reach and other efforts scale down, because `ReadAttenuationSettings`' captured ray/LoS ranges stay at base and scaling above 1 would make lines audible beyond where rays search. Tunables live in `UNPCVoiceSettings` (same asset-or-CDO pattern as `USpatialAudioSettings`).
